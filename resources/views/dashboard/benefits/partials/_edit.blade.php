    ...
    @foreach(LaravelLocalization::getSupportedLocales() as $locale => $properties)
        <div class="col-md-12">
            <div class="form-group" id="{{ $locale }}_description_edit_div">
                @php $description[$locale] = isset($form_data) ? $form_data->getTranslateName($locale) : ""; @endphp
                <label
                    for="{{ $locale }}_description_edit_input">@lang('site.' . $locale . '.description')</label>
                <textarea name="{{ $locale }}_description" class="form-control" id="{{ $locale }}_description_edit_input"
                >{{ $description[$locale] }}</textarea>
